...
class Simulation:
    ...
    def run(self, initial_events: List[Event]) -> Dict[str, float]:
        ...
        # while the event queue is not empty, iterate over it and remove the
        # event with the highest priority
        while not self._events.is_empty():
            new_event = self._events.remove()
            if isinstance(new_event, Event):
                # execute the event
                added_events = new_event.do(self._dispatcher, self._monitor)
                # iterate over all the added events and add them to the queue
    ...
    python_ta.check_all(
        config={
            'extra-imports': ['typing', 'container', 'dispatcher', 'event',
                              'monitor']})
    events = create_event_list("a1_my_events.txt ")
    sim = Simulation()
