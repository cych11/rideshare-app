...
class Monitor:
    ...
    def _average_wait_time(self) -> float:
        ...
        for activities in self._activities[RIDER].values():
            ...
            if len(activities) >= 2:
                ...
                count += 1
        if count == 0:  # ensure there is no ZeroDivisionError
            return 0
    ...
    def _average_total_distance(self) -> float:
        ...
        for activities in self._activities[DRIVER].values():
            # take all the drivers events and calculate the distance between
            # each of them, adding them to total_distance, then increasing
            # the count by 1, which refers to the amount of drivers
            for x in range(len(activities) - 1):
                total_distance += manhattan_distance(activities[x].location,
    ...
    def _average_ride_distance(self) -> float:
        ...
        for activities in self._activities[RIDER].values():
            # check if the 2nd last event is a pickup and the last event is a
            # drop-off, meaning the ride was successful.
            if (activities[1].description == PICKUP
                    and activities[2].description == DROPOFF):
                # if the ride was successful, calculate the distance between
                # the origin and destination, adding it to total_distance
                total_distance += manhattan_distance(
                    activities[1].location, activities[2].location)
                # increase the count by 1, which refers to the amount of riders
                count += 1
        if count == 0:  # ensure there is no ZeroDivisionError
            return 0
        return total_distance / count
